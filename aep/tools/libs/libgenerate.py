...
def simulate(
    ...
    pipeline, sim.backburner = fill_pipeline(techniques, tech_bundle, sim.provided)
    if not pipeline:
...
def fill_pipeline(
    techniques: Dict, potensial: List[Text], provided: Set[Text]
...
def stage_technique(
    technique: Dict, last_stage_sum_provides: Set[Text], show_tactics: bool
) -> Text:
    ...
    if all(provides in last_stage_sum_provides for provides in technique["provides"]):
        description += " [*]"
    ...
    for idx, stage in enumerate(sim.stages):
        ...
        for tech_id in stage.techniques:
            ...
            tech_descriptions.add(
                stage_technique(technique, stage.last_stage_sum_provides, show_tactics)
            )
